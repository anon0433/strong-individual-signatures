...
mnk <- read.csv(file.path(path, "focal_individual_sels_preprocessed.csv"), header = TRUE)
...
mnk_sites <- read.csv(file.path(path, "site-level_calls_preprocessed_final.csv"), header = TRUE)
...
#### Supplementary Figure 3: Important Parameter Types

We made bar graphs of variable importance across models (Supplementary Figure 3 in Supplementary Materials).
...
ggplot(data = var_type_df, aes(fct_rev(var_type))) + 
  ...
  guides(fill = FALSE) +
  theme_bw() + theme(legend.position = "top", legend.text = element_text(size = 9), axis.text = element_text(size = 10), axis.title = element_text(size = 14), strip.text = element_text(size = 14, face = "bold"))
  
# ggsave(file.path(path, "SuppFig3_ParameterTypes_2spp.jpeg"), units = "in", width = 7.48, height = 5.29, dpi = 300)
