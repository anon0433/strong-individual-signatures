...
ccs_fi <- read.csv(file.path(path, "Focal_Individuals/focal_individual_sels_preprocessed.csv"), header = TRUE)
...
ccs <- read.csv(file.path(path, "Site_Recordings/site-level_calls_preprocessed_final.csv"), header = TRUE)
...
xc_mat <- readRDS(file.path(path, "Site_Recordings/xc_mat_sites.RDS"))

proxm_site <- readRDS(file.path(path, "Site_Recordings/proxm_site.RDS"))

xc_mat_fi <- readRDS(file.path(path, "Focal_Individuals/xc_mat_allfocal.RDS"))
...
ggplot(corr_df, aes(x = distance, y = corr)) +
  ...
  geom_point(aes(color = sig_cat, fill = sig_cat), size = 2.5, shape = 21) +
  ...
  theme(panel.background = element_rect(color = "white", fill = "white"), panel.grid.major.x = element_line(size = 0.1, color = "black"), axis.line = element_line(color = "black", size = 0.35), axis.text.x = element_text(size = 8, angle = 0, face = "bold"), axis.title = element_text(size = 12), axis.ticks = element_line(size = 0.15), legend.text = element_text(hjust = 0, size = 12), legend.title = element_text(hjust = 0, size = 12), panel.grid.major.y = element_blank(), panel.grid.minor.y = element_blank(), legend.position = "top", strip.text = element_text(size = 10)) +
...
# ggsave("Mantel_correlogram.jpeg", width = 7.48, height = 5.29, units = "in", dpi = 300)
