...
This code can be reproduced by using the extended selection tables shared on GitHub for the individual and higher social scales, as well as the original selection tables for each. Some changes to this code will be necessary to use signals saved in the extended selection tables. There may be slight differences in some parameter calculations between signals used from extended selection tables and those we report here (signals within original recordings), but such differences should not change overall results. 
...
ccs_fi <- read.csv(file.path(path, "focal_individual_sels_preprocessed.csv"), header = TRUE)
# str(ccs_fi)
...
saveRDS(xc_mat, file.path(path, "xc_mat_allfocal.RDS"))
...
xc_mat <- readRDS(file.path(path, "xc_mat_allfocal.RDS"))
# str(xc_mat)
...
saveRDS(xc_mat_sites, file.path(path, "xc_mat_sites.RDS"))
...
# SPCC
xc_mat <- readRDS(file.path(path, "xc_mat_sites.RDS"))
# str(xc_mat)
